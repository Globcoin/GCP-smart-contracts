...
contract('GlobCoinTokenSale', function(accounts) {
  describe('Constructor', () => {
        ...
        // presale cap  30 000 ETH
        let presaleCap = 30000000000000000000000;
        // minimum amount of funds to be raised in weis - 10 000 ETH
        let goal = 10000000000000000000000;
        // Maximum amount of funds to be raised - 150 000 ETH
        let cap = 150000000000000000000000;
        // address where funds are collected
        let wallet = "0xcf09f36227aa07e3318fa57a16b453d29ecf786d";

        before(async () => {
          globCoinSaleInstance = await GlobCoinTokenSale.new(startBlock,startSale,endBlock,goal,presaleCap,cap,wallet, { from: accounts[0]});
        });
      ...
      describe('Sale Features', () => {
            ...
            // presale cap  30 000 ETH
            let presaleCap = 30000000000000000000000;
            // minimum amount of funds to be raised in weis - 25 000 ETH
            let goal = 25000000000000000000000;
            // Maximum amount of funds to be raised - 150 000 ETH
            let cap = 150000000000000000000000;
            // address where funds are collected
            let wallet = "0xcf09f36227aa07e3318fa57a16b453d29ecf786d";
            ...
            it( 'should give the team their tokens if the sale is sucessfull', async () => {
              ...
              await web3.eth.sendTransaction({ from: accounts[1], to: globCoinSaleInstance.address , value: "150000000000000000000000" ,gas: 4700000});
            ...
            it( 'should not give the team their tokens if the sale is unsucessfull', async () => {
              while (web3.eth.blockNumber < startSale) {
                await advanceBlock()
              }
            ...
            it('should allow refunds if the softcap isnt reached', async () => {
              ...
              await web3.eth.sendTransaction({ from: accounts[1], to: globCoinSaleInstance.address , value: "1000000000000000000000" ,gas: 4700000});
      ...
      describe('Sale Distribution 2', () => {
        // v10 globcoin
            ...
            it('should give 130 Token at the startSale Block of the sale for 1 wei.', async () => {
              //Refactor this to use advanceToBlock(startBlock);
              while (web3.eth.blockNumber < startSale) {
                await advanceBlock()
              }
            ...
            it('should give 13000000 Token at the startSale Block of the sale for 100000 wei.', async () => {
              //Refactor this to use advanceToBlock(startBlock);
              while (web3.eth.blockNumber < startSale) {
                await advanceBlock()
              }
              ...
              let balance = await gloInstance.balanceOf(accounts[1]);
              assert.equal(balance, 13000000);
            });

            it('should give 1300000000000000000000120 Token at the startSale of the sale for 10000000000000000000001 wei.', async () => {
              //Refactor this to use advanceToBlock(startBlock);
              while (web3.eth.blockNumber < startSale) {
                await advanceBlock()
              }
              await globCoinSaleInstance.sendTransaction({ from: accounts[1], to: globCoinSaleInstance.address , value: "10000000000000000000001" ,gas: 4700000});
              let balance = await gloInstance.balanceOf(accounts[1]);
              assert.equal(balance, 1300000000000000000000120);
            });


            it('should give 1299999999999999999999870 Token at the startSale of the sale for 9999999999999999999999 wei.', async () => {
              //Refactor this to use advanceToBlock(startBlock);
              while (web3.eth.blockNumber < startSale) {
                await advanceBlock()
              }
              await globCoinSaleInstance.sendTransaction({ from: accounts[1], to: globCoinSaleInstance.address , value: "9999999999999999999999" ,gas: 4700000});
              let balance = await gloInstance.balanceOf(accounts[1]);
              ...
              assert.equal(balance, 1299999999999999999999870);

            });
            it('should give 1300000000000000000000000 Token at the startSale of the sale for 10000000000000000000000 wei.', async () => {
              //Refactor this to use advanceToBlock(startBlock);
              while (web3.eth.blockNumber < startSale) {
                await advanceBlock()
              }
        ...
        describe('Presale Features', () => {
              ...
              it('should buy the whole presale', async () => {
                ...
                await globCoinSaleInstance.whitelistAddressesPresale(new Array(accounts[1]), { from: accounts[0]});
                await globCoinSaleInstance.sendTransaction({ from: accounts[1], to: globCoinSaleInstance.address , value: "30000000000000000000000" ,gas: 4700000});
                let balance = await gloInstance.balanceOf(accounts[1]);
                assert.equal(balance, 5100000000000000000000000);
              });
